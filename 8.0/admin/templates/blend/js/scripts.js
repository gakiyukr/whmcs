...
var blendIntelliSearch = {
    ...
    inputExpand: function() {
        var $targetOffset = $(this.refs.form).offset();
        $(this.refs.form).data('leftpos', $targetOffset.left)
            .css({
                position: 'absolute',
        ...
        if ($('#btnNavbarToggle').is(':visible')) {
            $(this.refs.form).delay(10).queue(function (next) {
                    $(this).addClass('active full-width');
                    next();
                });
        } else {
            $(this.refs.form).addClass('active')
    ...
    close: function() {
        ...
        }).removeClass('active full-width').delay(500).queue(function (next) {
            $form.css({
                position: '',
                top: '',
                left: ''
...
function () {
    ...
    this.clientSearch = function () {
        ...
        selectElement.each(function (){
            ...
            var instance = module.clients(selector, undefined, configuration);
...
function slugify(text) {
    var search =  "ãàáäâẽèéëêìíïîõòóöôùúüûñç·/_,:;";
    var replace = "aaaaaeeeeeiiiiooooouuuunc------";
